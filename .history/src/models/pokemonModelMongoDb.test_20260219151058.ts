...
test("Can add pokemon to DB (external)", async () => {
  ...
  expect(await model.addPokemon(newPokemon.name, newPokemon.type)).toEqual({
    name: newPokemon.name,
    type: newPokemon.type,
  });

  await expect(model.addPokemon("3mon2", newPokemon.type)).rejects.toThrow(
    InvalidInputError,
  );

  await expect(model.addPokemon(newPokemon.name, "Red Firework")).rejects.toThrow(
    InvalidInputError,
  );
});
