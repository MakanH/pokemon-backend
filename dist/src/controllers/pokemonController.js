...
async function readPokemon(request, response) {
    try {
        ...
        response
            .status(200)
            .send(`Successfully found pokemon. Name: ${foundPokemon.name}, Type: ${foundPokemon.type}`);
    }
    catch (error) {
...
async function createPokemon(request, response) {
    try {
        ...
        response
            .status(201)
            .send(`Successfully added pokemon. Name: ${newPokemon.name}, Type: ${newPokemon.type}`);
    }
    catch (error) {
        if (error instanceof InvalidInputError) {
            response.status(400).send(`User Error: ${error.message}`);
