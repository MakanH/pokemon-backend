 ...
 */
async function initialize(dbFilename, resetFlag, url) {
    try {
        logger.debug("Starting the database initialization process.");
        logger.trace(`Initialization parameters received - dbFilename: ${dbFilename}, resetFlag: ${resetFlag}`);
        client = new MongoClient(url); // store connected client for use while the app is running
        await client.connect();
        logger.info("Connected to MongoDb");
        const db = client.db(dbFilename);
...
async function close() {
    try {
        ...
        if (err instanceof Error)
            ...
            console.log("Unknown error during beforeEach in unit tests");
    }
}
/**
...
async function addPokemon(name, type) {
    ...
    if (!pokemonsCollection)
        throw new DatabaseError("Error when adding pokemon - Collection not initialized");
    try {
        const pokemonToAdd = { name: name, type: type };
        console.log(`Inserted pokemon ${(await pokemonsCollection.insertOne(pokemonToAdd)).insertedId}`);
        return pokemonToAdd;
    }
    catch (error) {
...
async function getSinglePokemon(name) {
    if (!pokemonsCollection)
        throw new DatabaseError("Error when getting a pokemon - Collection not initialized");
    try {
        const found = await pokemonsCollection.findOne({ name: name });
        if (!found)
            throw new DatabaseError(`Couldn't find pokemon ${name}`);
        return found;
    }
    catch (error) {
        if (error instanceof Error) {
            console.error(`Unexpected error when getting a pokemon: ${error.message}`);
            throw new DatabaseError(`Unexpected error when getting a pokemon: ${error.message}`);
        }
...
async function getAllPokemons() {
    if (!pokemonsCollection)
        throw new DatabaseError("Error when getting all pokemon - Collection not initialized");
    try {
        return await await pokemonsCollection.find({}).toArray();
    }
    catch (error) {
        if (error instanceof Error) {
            console.error(`Unexpected error when getting all pokemon: ${error.message}`);
            throw new DatabaseError(`Unexpected error when getting all pokemon: ${error.message}`);
        }
 ...
 */
async function updatePokemon(name, type, newName, newType) {
    isValid(newName, newType);
    if (!pokemonsCollection)
        throw new DatabaseError("Error when updating pokemon - Collection not initialized");
    try {
        const result = await pokemonsCollection.updateOne({ name: name, type: type }, { $set: { name: newName, type: newType } });
        if (result.matchedCount === 0) {
            throw new DatabaseError(`Couldn't find pokemon ${name} with type ${type}`);
        }
        ...
        if (error instanceof Error) {
            ...
            throw new DatabaseError(`Unexpected error when updating pokemon: ${error.message}`);
        }
 ...
 */
async function deletePokemon(name, type) {
    if (!pokemonsCollection)
        throw new DatabaseError("Error when deleting pokemon - Collection not initialized");
    try {
        ...
        if (error instanceof Error) {
            ...
            throw new DatabaseError(`Unexpected error when deleting pokemon: ${error.message}`);
        }
...
//# sourceMappingURL=pokemonModelMongoDb.js.map